...
(() => {
  ...
  function favButtonHtml(item) {
    const active = isFav(item.id);
    const label = esc(t("card.fav"));
    return `<button class="card__fav${active ? " is-active" : ""}" type="button" title="${label}" aria-label="${label}" aria-pressed="${active ? "true" : "false"}">
      <svg class="card__fav-icon" viewBox="0 0 24 24" aria-hidden="true">
